...
export const withTasksMethods = () => {
  return signalStoreFeature({ state: type<TasksStoreState>() }, withMethods((store) => {
     ...
     */
    const getTasks = rxMethod<void>(pipe(
      tap(() => patchState(store, { isLoading: true })),
      switchMap(() => httpService.getAllTasks$().pipe(tapResponse({
     ...
     */
    const removeTask = rxMethod<string>(pipe(
      tap(() => patchState(store, { isLoading: true })),
      switchMap(id => httpService.removeTask$(id).pipe(tapResponse({
     ...
     */
    const addTask = rxMethod<Task>(pipe(
      switchMap(task => httpService.addTask$(task).pipe(tapResponse({
